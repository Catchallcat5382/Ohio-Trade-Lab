...
export default {async fetch(req,env){
 ...
 try{
  ...
  m=path.match(/^\/rooms\/([^/]+)\/action$/);if(m&&req.method==="POST"){
   ...
   if(action==='cancel'||action==='shutdown'){await env.DB.batch([env.DB.prepare("UPDATE listings SET status='cancelled',completed=? WHERE id=? AND status='accepted'").bind(Date.now(),room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,action==='shutdown'?'Room shut down. Trade cancelled.':'Trade cancelled.',Date.now()),env.DB.prepare("UPDATE rooms SET status='closed' WHERE id=?").bind(room.id)]);return json({ok:true,status:'cancelled'});}
