...
const adminCookie=(token,maxAge=1800)=>`otl_admin=${encodeURIComponent(token)}; Path=/api/admin; HttpOnly; Secure; SameSite=Strict; Max-Age=${maxAge}`;
...
async function adminTokenFor(env,user){const payload=b64u(enc.encode(JSON.stringify({sub:user.id,scope:'admin',exp:Date.now()+30*60*1000})));return payload+'.'+b64u(await hmac(env.SESSION_SECRET,payload))}
async function adminUnlocked(req,env,user){const raw=decodeURIComponent(cookieValue(req,'otl_admin')||'');if(!raw||!user)return false;const [p,s]=raw.split('.');if(!p||!s)return false;const expected=await hmac(env.SESSION_SECRET,p);const got=unb64u(s);if(expected.length!==got.length)return false;let ok=true;for(let i=0;i<got.length;i++)ok&&=expected[i]===got[i];if(!ok)return false;try{const d=JSON.parse(new TextDecoder().decode(unb64u(p)));return d.scope==='admin'&&d.sub===user.id&&d.exp>Date.now()}catch{return false}}
...
export default {async fetch(req,env){
 ...
 try{
  ...
  if(path==="/auth/profile"&&req.method==="POST"){const b=await req.json(),display=clean(b.displayName,24),bio=clean(b.bio,180),avatar=clean(b.avatar,700);if(display.length<2||blocked(display+' '+bio))return json({error:"Invalid profile"},400);if(avatar&&!/^https:\/\//i.test(avatar)&&!avatar.startsWith('/assets/'))return json({error:"Profile image must use a secure HTTPS URL"},400);await env.DB.prepare("UPDATE users SET display_name=?,bio=?,avatar=? WHERE id=?").bind(display,bio,avatar||'/assets/default-avatar.svg',me.id).run();const u=await env.DB.prepare("SELECT * FROM users WHERE id=?").bind(me.id).first();return json({user:publicUser(u)});}
  ...
  m=path.match(/^\/admin\/listings\/([^/]+)$/);if(m&&req.method==="DELETE"){if(!adminOK)return json({error:"Developer panel locked. Enter the separate access code."},403);await env.DB.prepare("UPDATE listings SET status='removed' WHERE id=?").bind(m[1]).run();return json({ok:true});}
