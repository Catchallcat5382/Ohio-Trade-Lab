...
export default {async fetch(req,env){
 ...
 try{
  ...
  if(path==="/listings"&&req.method==="GET"){
   const {results}=await env.DB.prepare("SELECT l.*,u.display_name owner_display FROM listings l JOIN users u ON u.id=l.owner_id WHERE l.status IN ('open','accepted','completed','cancelled','delivery_pending','ended_no_bids') ORDER BY l.created DESC LIMIT 300").all();
  ...
  let m=path.match(/^\/listings\/([^/]+)$/);if(m&&req.method==="DELETE"){
   ...
   if(row.status!=='open'||bidCount>0||roomCount>0||row.accepted_by)return json({error:"This listing has an offer, bid, or active room. Open the room and cancel it there instead."},409);
  ...
  m=path.match(/^\/listings\/([^/]+)\/accept$/);if(m&&req.method==="POST"){
   const row=await env.DB.prepare("SELECT * FROM listings WHERE id=? AND type='trade'").bind(m[1]).first();if(!row)return json({error:"Trade not found"},404);if(row.owner_id===me.id)return json({error:"Cannot accept your own trade"},400);if(row.status!=='open')return json({error:"This trade is already pending, sold, cancelled, or unavailable"},409);
   const claim=await env.DB.prepare("UPDATE listings SET status='accepted',accepted_by=? WHERE id=? AND type='trade' AND status='open'").bind(me.id,row.id).run();if(Number(claim?.meta?.changes||0)!==1)return json({error:"Someone else accepted this trade first"},409);
   const room=uid(),created=Date.now();try{await env.DB.batch([env.DB.prepare("INSERT INTO rooms(id,listing_id,title,member_a,member_b,created,status) VALUES(?,?,?,?,?,?,?)").bind(room,row.id,row.title,row.owner_id,me.id,created,"open"),env.DB.prepare("INSERT INTO notifications(id,recipient_id,body,room_id,created) VALUES(?,?,?,?,?)").bind(uid(),row.owner_id,`${me.display_name} accepted your trade. Open the room to confirm, deny, or cancel.`,room,created),env.DB.prepare("INSERT INTO notifications(id,recipient_id,body,room_id,created) VALUES(?,?,?,?,?)").bind(uid(),me.id,`Trade room opened for ${row.title}.`,room,created)]);}catch(e){await env.DB.prepare("UPDATE listings SET status='open',accepted_by=NULL WHERE id=? AND accepted_by=?").bind(row.id,me.id).run();throw e}return json({ok:true,room,status:'accepted'});
  }
  m=path.match(/^\/listings\/([^/]+)\/bid$/);if(m&&req.method==="POST"){
   ...
   const old=await env.DB.prepare("SELECT id FROM bids WHERE listing_id=? AND bidder_id=? ORDER BY created DESC LIMIT 1").bind(row.id,me.id).first(),bidId=old?.id||uid(),created=Date.now();
   if(old)await env.DB.prepare("UPDATE bids SET items_json=?,value=?,created=?,demand_score=?,effective_score=? WHERE id=?").bind(JSON.stringify(items),value,created,demandScore,effectiveScore,bidId).run();
   else await env.DB.prepare("INSERT INTO bids(id,listing_id,bidder_id,items_json,value,created,demand_score,effective_score) VALUES(?,?,?,?,?,?,?,?)").bind(bidId,row.id,me.id,JSON.stringify(items),value,created,demandScore,effectiveScore).run();
   await env.DB.prepare("INSERT INTO notifications(id,recipient_id,body,room_id,created) VALUES(?,?,?,?,?)").bind(uid(),row.owner_id,me.display_name+(old?' updated':' submitted')+' an anonymous offer on '+row.title+'. A private room opens only for the winning bidder after the timer ends.',null,created).run();
  ...
  m=path.match(/^\/rooms\/([^/]+)\/action$/);if(m&&req.method==="POST"){
   ...
   if(action==='confirm'){if(room.type!=='trade'||room.listing_status!=='accepted')return json({error:"Trade is not awaiting confirmation"},409);await decrementInventory(env,room.owner_id,JSON.parse((await env.DB.prepare("SELECT give_json FROM listings WHERE id=?").bind(room.listing_id).first()).give_json||'[]'));const lr=await env.DB.prepare("SELECT want_json FROM listings WHERE id=?").bind(room.listing_id).first();if(room.accepted_by)await decrementInventory(env,room.accepted_by,JSON.parse(lr.want_json||'[]'));await env.DB.batch([env.DB.prepare("UPDATE listings SET status='completed',completed=? WHERE id=? AND status='accepted'").bind(Date.now(),room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'Trade confirmed. Listing marked SOLD.',Date.now()),env.DB.prepare("UPDATE rooms SET status='closed' WHERE id=?").bind(room.id)]);return json({ok:true,status:'completed'});}
   if(action==='deny'){if(me.id!==room.owner_id)return json({error:"Only the trade owner can deny and reopen this trade"},403);await env.DB.batch([env.DB.prepare("UPDATE listings SET status='open',accepted_by=NULL WHERE id=? AND status='accepted'").bind(room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'Trade denied. Listing reopened.',Date.now()),env.DB.prepare("UPDATE rooms SET status='closed' WHERE id=?").bind(room.id)]);return json({ok:true,status:'open'});}
   ...
   if(action==='cancel'){
    ...
    await env.DB.batch([env.DB.prepare("UPDATE listings SET status='open',accepted_by=NULL WHERE id=? AND status='accepted'").bind(room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'This match was cancelled. The trade listing remains active for other users.',now),env.DB.prepare("UPDATE rooms SET status='closed' WHERE id=?").bind(room.id)]);return json({ok:true,status:'open'});
   }
