...
export default {async fetch(req,env){
 ...
 try{
  ...
  if(path==="/listings"&&req.method==="GET"){
   const {results}=await env.DB.prepare("SELECT l.*,u.username owner_username,u.display_name owner_display FROM listings l JOIN users u ON u.id=l.owner_id WHERE l.status IN ('open','accepted','completed','cancelled','delivery_pending','ended_no_bids') ORDER BY l.created DESC LIMIT 300").all();
  ...
  m=path.match(/^\/listings\/([^/]+)\/bid$/);if(m&&req.method==="POST"){
   const b=await req.json(),items=Array.isArray(b.items)?b.items.slice(0,8).map(x=>clean(x,120)).filter(Boolean):[],value=Math.floor(Number(b.value)||0),row=await env.DB.prepare("SELECT * FROM listings WHERE id=? AND type='auction' AND status='open' AND expires>?").bind(m[1],Date.now()).first();
   ...
   const existing=await env.DB.prepare("SELECT id FROM rooms WHERE listing_id=? AND member_a=? AND member_b=? AND status='open' LIMIT 1").bind(row.id,row.owner_id,me.id).first();
   if(existing)return json({error:"You already have an open offer room for this auction. Open that room to continue.",room:existing.id},409);
   const bidId=uid(),room=uid(),created=Date.now();
   await env.DB.batch([
    env.DB.prepare("INSERT INTO bids(id,listing_id,bidder_id,items_json,value,created) VALUES(?,?,?,?,?,?)").bind(bidId,row.id,me.id,JSON.stringify(items),value,created),
    env.DB.prepare("INSERT INTO rooms(id,listing_id,title,member_a,member_b,created,status) VALUES(?,?,?,?,?,?,?)").bind(room,row.id,'Auction offer: '+row.title,row.owner_id,me.id,created,'open'),
    env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room,me.id,'Auction offer submitted with '+items.length+' item(s), total value '+value+'.',created),
    env.DB.prepare("INSERT INTO notifications(id,recipient_id,body,room_id,created) VALUES(?,?,?,?,?)").bind(uid(),row.owner_id,me.display_name+' submitted an offer on '+row.title+'. Open the room to discuss, accept, or cancel.',room,created)
   ]);
   return json({ok:true,room,bidId});
  }
  ...
  m=path.match(/^\/rooms\/([^/]+)\/action$/);if(m&&req.method==="POST"){
   ...
   if(action==='cancel'||action==='shutdown'){
    const now=Date.now();
    ...
    await env.DB.batch([env.DB.prepare("UPDATE listings SET status='cancelled',completed=? WHERE id=? AND status='accepted'").bind(now,room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,action==='shutdown'?'Room shut down. Trade cancelled.':'Trade cancelled.',now),env.DB.prepare("UPDATE rooms SET status='closed' WHERE id=?").bind(room.id)]);return json({ok:true,status:'cancelled'});
   }
