...
const publicUser=u=>({id:u.id,username:u.username,displayName:u.display_name,avatar:u.avatar||"/assets/default-avatar.svg",bio:u.bio||'',role:u.role,needsDisplayName:Boolean(Number(u.needs_display_name||0))});
...
async function ensureCoreSchema(env){
 const statements=[
  `CREATE TABLE IF NOT EXISTS users (
    ...
    needs_display_name INTEGER NOT NULL DEFAULT 0
  )`,
 ...
 try{
  ...
  if(path==="/auth/discord/callback"&&req.method==="GET"){
   try{
    ...
    const ur=await fetch("https://discord.com/api/users/@me",{headers:{authorization:`Bearer ${td.access_token}`}});const g=await ur.json().catch(()=>({}));if(!ur.ok||!g.id)return oauthFail(env,url,"Discord could not verify this Discord account.");
    const email=`discord-${g.id}@oauth.invalid`,avatar=g.avatar?`https://cdn.discordapp.com/avatars/${g.id}/${g.avatar}.png?size=128`:"/assets/default-avatar.svg";
    ...
    u=await applyVerifiedOwnerRole(env,u,email,false);
  ...
  if(path==="/listings"&&req.method==="POST"){
   ...
   if(Array.isArray(b.inventorySnapshot))await replaceInventory(env,me.id,b.inventorySnapshot);const owned=(await env.DB.prepare("SELECT item_id,quantity FROM user_inventory WHERE user_id=? AND quantity>0").bind(me.id).all()).results;const ownMap=new Map(owned.map(x=>[String(x.item_id),Number(x.quantity)]));const counts={};for(const id of give)counts[id]=(counts[id]||0)+1;for(const [id,q] of Object.entries(counts))if((ownMap.get(id)||0)<q)return json({error:"One or more items you are listing are missing from your synced Inventory. Refresh the Inventory tab and try again."},400);
   ...
   const created=Date.now(),expires=type==='trade'?0:Math.min(created+7*864e5,Math.max(created+36e5,Number(b.expires)||created+864e5)),id=uid();await env.DB.prepare("INSERT INTO listings(id,type,owner_id,title,description,give_json,want_json,category,preferred_section,preferred_set,min_bid_value,max_bid_value,buyout_json,created,expires,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(id,type,me.id,title,description,JSON.stringify(give),JSON.stringify(want),clean(b.category,20),clean(b.preferredSection,40),clean(b.preferredSet,40),minBid,maxBid,JSON.stringify(buyout),created,expires,"open").run();return json({ok:true,id});
  }
  let m=path.match(/^\/listings\/([^/]+)$/);if(m&&req.method==="DELETE"){
   const row=await env.DB.prepare("SELECT * FROM listings WHERE id=? AND owner_id=?").bind(m[1],me.id).first();
   if(!row)return json({error:"Listing not found or you do not own it"},404);
   if(!['open','delivery_pending'].includes(row.status))return json({error:"This listing is already finished or cancelled"},409);
   const bidCount=Number((await env.DB.prepare("SELECT COUNT(*) n FROM bids WHERE listing_id=?").bind(row.id).first())?.n||0);
   const roomCount=Number((await env.DB.prepare("SELECT COUNT(*) n FROM rooms WHERE listing_id=?").bind(row.id).first())?.n||0);
   if(bidCount===0&&roomCount===0&&row.status==='open'){
    await env.DB.prepare("DELETE FROM notifications WHERE body LIKE ?").bind('%'+row.title+'%').run();
    const result=await env.DB.prepare("DELETE FROM listings WHERE id=? AND owner_id=?").bind(row.id,me.id).run();
    return json({ok:true,deleted:true,changes:Number(result?.meta?.changes||0)});
   }
   const now=Date.now();await env.DB.batch([env.DB.prepare("UPDATE listings SET status='cancelled',completed=? WHERE id=? AND owner_id=?").bind(now,row.id,me.id),env.DB.prepare("UPDATE rooms SET status='closed' WHERE listing_id=?").bind(row.id)]);
   return json({ok:true,deleted:false,cancelled:true});
  }
  m=path.match(/^\/listings\/([^/]+)\/accept$/);if(m&&req.method==="POST"){
   const row=await env.DB.prepare("SELECT * FROM listings WHERE id=? AND type='trade'").bind(m[1]).first();if(!row)return json({error:"Trade not found"},404);if(row.owner_id===me.id)return json({error:"Cannot open an offer room on your own trade"},400);if(row.status!=='open')return json({error:"This trade is sold, cancelled, or unavailable"},409);
   let existing=await env.DB.prepare("SELECT id FROM rooms WHERE listing_id=? AND member_a=? AND member_b=? AND status='open' ORDER BY created DESC LIMIT 1").bind(row.id,row.owner_id,me.id).first();
   if(existing)return json({ok:true,room:existing.id,status:'open',existing:true});
   const room=uid(),created=Date.now();await env.DB.batch([env.DB.prepare("INSERT INTO rooms(id,listing_id,title,member_a,member_b,created,status) VALUES(?,?,?,?,?,?,?)").bind(room,row.id,row.title,row.owner_id,me.id,created,"open"),env.DB.prepare("INSERT INTO notifications(id,recipient_id,body,room_id,created) VALUES(?,?,?,?,?)").bind(uid(),row.owner_id,`${me.display_name} opened a chat and offer room for ${row.title}. The trade remains active for everyone until you finish or cancel it.`,room,created),env.DB.prepare("INSERT INTO notifications(id,recipient_id,body,room_id,created) VALUES(?,?,?,?,?)").bind(uid(),me.id,`Chat and offer room opened for ${row.title}.`,room,created)]);return json({ok:true,room,status:'open'});
  }
  m=path.match(/^\/listings\/([^/]+)\/end-early$/);if(m&&req.method==="POST"){
   const row=await env.DB.prepare("SELECT * FROM listings WHERE id=? AND type='auction' AND owner_id=? AND status='open'").bind(m[1],me.id).first();if(!row)return json({error:"Active auction not found or you do not own it"},404);
   await env.DB.prepare("UPDATE listings SET expires=? WHERE id=? AND status='open'").bind(Date.now(),row.id).run();await finalizeAuctions(env);
   const finished=await env.DB.prepare("SELECT status FROM listings WHERE id=?").bind(row.id).first();const room=await env.DB.prepare("SELECT id FROM rooms WHERE listing_id=? AND status='open' ORDER BY created DESC LIMIT 1").bind(row.id).first();return json({ok:true,status:finished?.status||'ended_no_bids',room:room?.id||null});
  }
  m=path.match(/^\/listings\/([^/]+)\/bid$/);if(m&&req.method==="POST"){
   const b=await req.json(),items=Array.isArray(b.items)?b.items.slice(0,8).map(x=>clean(x,120)).filter(Boolean):[],value=Math.floor(Number(b.value)||0),demandScore=Math.max(0,Math.min(10,Number(b.demandScore)||0)),effectiveScore=Math.max(0,Math.floor(Number(b.effectiveScore)||value)),row=await env.DB.prepare("SELECT * FROM listings WHERE id=? AND type='auction' AND status='open' AND expires>?").bind(m[1],Date.now()).first();
   if(!row)return json({error:"Auction unavailable"},404);if(row.owner_id===me.id)return json({error:"Cannot offer on your own auction"},400);if(!items.length||value<row.min_bid_value||(row.max_bid_value>0&&value>row.max_bid_value))return json({error:"Offer does not meet auction value requirements"},400);
   const ownedRows=(await env.DB.prepare("SELECT item_id,quantity FROM user_inventory WHERE user_id=? AND quantity>0").bind(me.id).all()).results,ownedMap=new Map(ownedRows.map(x=>[String(x.item_id),Number(x.quantity)])),bidCounts={};
   for(const id of items)bidCounts[id]=(bidCounts[id]||0)+1;for(const [id,q] of Object.entries(bidCounts))if((ownedMap.get(id)||0)<q)return json({error:"You can only offer items currently in your inventory"},400);
   const old=await env.DB.prepare("SELECT id,items_json,value FROM bids WHERE listing_id=? AND bidder_id=? ORDER BY created DESC LIMIT 1").bind(row.id,me.id).first(),created=Date.now();
   if(old){if(value<=Number(old.value||0))return json({error:`Your new bid must be higher than your last bid of ${Number(old.value||0).toLocaleString()}.`},409);const prev=JSON.parse(old.items_json||'[]'),need={};for(const id of prev)need[id]=(need[id]||0)+1;const have={};for(const id of items)have[id]=(have[id]||0)+1;for(const [id,q] of Object.entries(need))if((have[id]||0)<q)return json({error:"A new bid must add onto your previous bid. You cannot remove items you already offered."},409);}
   const bidId=uid();await env.DB.prepare("INSERT INTO bids(id,listing_id,bidder_id,items_json,value,created,demand_score,effective_score) VALUES(?,?,?,?,?,?,?,?)").bind(bidId,row.id,me.id,JSON.stringify(items),value,created,demandScore,effectiveScore).run();
   await env.DB.prepare("INSERT INTO notifications(id,recipient_id,body,room_id,created) VALUES(?,?,?,?,?)").bind(uid(),row.owner_id,me.display_name+' submitted an anonymous offer on '+row.title+'. A private room opens only for the winning bidder after the timer ends.',null,created).run();
   return json({ok:true,bidId,updated:Boolean(old),room:null});
  }
  ...
  if(path==="/rooms"&&req.method==="GET"){const {results}=await env.DB.prepare("SELECT r.id,r.listing_id listingId,r.title,r.created,r.status,l.status listingStatus,l.owner_id ownerId,l.accepted_by acceptedBy FROM rooms r LEFT JOIN listings l ON l.id=r.listing_id WHERE r.member_a=? OR r.member_b=? ORDER BY r.created DESC LIMIT 100").bind(me.id,me.id).all();return json({rooms:results});}
  m=path.match(/^\/rooms\/([^/]+)\/action$/);if(m&&req.method==="POST"){
   const room=await env.DB.prepare("SELECT r.*,l.type,l.status listing_status,l.owner_id,l.accepted_by FROM rooms r LEFT JOIN listings l ON l.id=r.listing_id WHERE r.id=? AND (r.member_a=? OR r.member_b=?)").bind(m[1],me.id,me.id).first();if(!room)return json({error:"Private room not found"},404);if(room.status!=='open')return json({error:"This room is already closed"},409);const b=await req.json(),action=String(b.action||'');
   if(action==='confirm'){if(room.type!=='trade'||room.listing_status!=='open')return json({error:"Trade is not active"},409);if(me.id!==room.owner_id)return json({error:"Only the trade owner can finish this trade"},403);await decrementInventory(env,room.owner_id,JSON.parse((await env.DB.prepare("SELECT give_json FROM listings WHERE id=?").bind(room.listing_id).first()).give_json||'[]'));const lr=await env.DB.prepare("SELECT want_json FROM listings WHERE id=?").bind(room.listing_id).first();await decrementInventory(env,room.member_b,JSON.parse(lr.want_json||'[]'));await env.DB.batch([env.DB.prepare("UPDATE listings SET status='completed',accepted_by=?,completed=? WHERE id=? AND status='open'").bind(room.member_b,Date.now(),room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'Trade confirmed. Listing marked SOLD.',Date.now()),env.DB.prepare("UPDATE rooms SET status='closed' WHERE listing_id=?").bind(room.listing_id)]);return json({ok:true,status:'completed'});}
   if(action==='deny'){if(me.id!==room.owner_id)return json({error:"Only the trade owner can deny this offer"},403);await env.DB.batch([env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'Offer denied. This chat is now closed; the trade remains active.',Date.now()),env.DB.prepare("UPDATE rooms SET status='closed' WHERE id=?").bind(room.id)]);return json({ok:true,status:'open'});}
   if(action==='accept_offer'){
    if(room.type!=='auction'||me.id!==room.owner_id||room.listing_status!=='open')return json({error:"Only the auction owner can accept an active offer"},403);
    const bid=await env.DB.prepare("SELECT * FROM bids WHERE listing_id=? AND bidder_id=? ORDER BY created DESC LIMIT 1").bind(room.listing_id,room.member_b).first();if(!bid)return json({error:"Offer not found"},404);
    await env.DB.batch([env.DB.prepare("UPDATE listings SET status='delivery_pending',winning_bid_id=?,accepted_by=? WHERE id=? AND status='open'").bind(bid.id,bid.bidder_id,room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'Auction offer accepted. Complete delivery in this room.',Date.now())]);return json({ok:true,status:'delivery_pending'});
   }
   if(action==='cancel'){
    const now=Date.now();
    if(room.type==='auction'){
     if(me.id!==room.owner_id&&me.id!==room.member_b)return json({error:"Not allowed"},403);
     if(me.id===room.owner_id){await env.DB.batch([env.DB.prepare("UPDATE listings SET status='cancelled',completed=? WHERE id=? AND status IN ('open','delivery_pending')").bind(now,room.listing_id),env.DB.prepare("UPDATE rooms SET status='closed' WHERE listing_id=?").bind(room.listing_id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,action==='shutdown'?'Auction room shut down and auction cancelled.':'Auction cancelled by owner.',now)]);return json({ok:true,status:'cancelled'});}
     await env.DB.batch([env.DB.prepare("DELETE FROM bids WHERE listing_id=? AND bidder_id=?").bind(room.listing_id,me.id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'Bidder withdrew this auction offer.',now),env.DB.prepare("UPDATE rooms SET status='closed' WHERE id=?").bind(room.id)]);return json({ok:true,status:'offer_withdrawn'});
    }
    if(me.id!==room.owner_id)return json({error:"Only the person who created this trade can cancel it"},403);
    await env.DB.batch([env.DB.prepare("UPDATE listings SET status='cancelled',completed=? WHERE id=? AND owner_id=? AND status='open'").bind(now,room.listing_id,me.id),env.DB.prepare("INSERT INTO messages(id,room_id,author_id,body,created,system) VALUES(?,?,?,?,?,1)").bind(uid(),room.id,me.id,'Trade cancelled by its creator. All related chats are now closed.',now),env.DB.prepare("UPDATE rooms SET status='closed' WHERE listing_id=?").bind(room.listing_id)]);return json({ok:true,status:'cancelled'});
   }
  ...
  m=path.match(/^\/rooms\/([^/]+)$/);if(m&&req.method==="GET"){const room=await env.DB.prepare("SELECT r.*,l.type listing_type,l.status listing_status,l.owner_id,l.accepted_by FROM rooms r LEFT JOIN listings l ON l.id=r.listing_id WHERE r.id=? AND (r.member_a=? OR r.member_b=?)").bind(m[1],me.id,me.id).first();if(!room)return json({error:"Private room"},403);const members=await env.DB.prepare("SELECT id,username,display_name FROM users WHERE id IN (?,?)").bind(room.member_a,room.member_b).all();const msgs=await env.DB.prepare("SELECT m.author_id,u.display_name authorDisplay,m.body,m.created,m.system FROM messages m JOIN users u ON u.id=m.author_id WHERE room_id=? ORDER BY m.created ASC LIMIT 500").bind(room.id).all();return json({room:{id:room.id,listingId:room.listing_id,listingType:room.listing_type,listingStatus:room.listing_status,ownerId:room.owner_id,isOwner:room.owner_id===me.id,title:room.title,status:room.status,membersDisplay:members.results.map(x=>x.display_name),messages:msgs.results}});}
